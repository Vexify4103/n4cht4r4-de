...
async function discordRequest<T>(config: DiscordConfig, path: string, method: string, body?: Record<string, unknown>) {
...
export async function enqueueDiscordJob(db: Db, type: DiscordQueueJobType, tournamentId: string, teamId: string, payload: Record<string, unknown> = {}, runAfter = new Date()) {
...
async function claimNextDiscordJob(db: Db) {
	...
	await db.collection<DiscordQueueJob>("discord_queue").updateMany(
		{ status: "processing", lockedAt: { $lt: staleBefore } },
		{ $set: { status: "queued", runAfter: new Date(), updatedAt: new Date() }, $unset: { lockedAt: "" } }
	);
	return db.collection<DiscordQueueJob>("discord_queue").findOneAndUpdate(
...
async function processTeamJob(db: Db, job: DiscordQueueJob, config: DiscordConfig) {
	...
	if (job.type === "team.create-text-channel") {
		...
		const channel = await discordRequest<{ id: string }>(config, `/guilds/${config.guildId}/channels`, "POST", {
			name: channelSlug(team.name as string, "chat"), type: 0, parent_id: config.categoryId,
			permission_overwrites: [{ id: config.guildId, type: 0, deny: "1024" }, { id: discord.roleId, type: 0, allow: "3072" }],
		});
	...
	if (job.type === "team.create-voice-channel") {
		...
		const channel = await discordRequest<{ id: string }>(config, `/guilds/${config.guildId}/channels`, "POST", {
			name: channelSlug(team.name as string, "voice"), type: 2, parent_id: config.categoryId,
			permission_overwrites: [{ id: config.guildId, type: 0, deny: "1024" }, { id: discord.roleId, type: 0, allow: "1049600" }],
		});
	...
	if (job.type === "team.assign-member-role") {
...
export async function processNextDiscordQueueJob(db: Db) {
	...
	if (!config) return { processed: false, reason: "Discord queue is not configured." };
	const job = await claimNextDiscordJob(db);
	if (!job) return { processed: false, reason: "No due jobs." };

	try {
		const status = await processTeamJob(db, job, config);
		...
		await db.collection<DiscordQueueJob>("discord_queue").updateOne({ id: job.id }, {
