...
export type WatchTimeResult = {
  ...
  reason?:
    ...
    | "stream_offline"
    | "chatters_credentials_missing"
    | "chatters_unavailable";
  matchedUsers: number;
...
async function saveChatterCredentials(
  accessToken: string,
  expiresAt: Date,
  refreshToken?: string
) {
...
function userMatchesChatter(user: Record<string, unknown>, chatters: TwitchChatter[], accountUserIds: Set<string>) {
  return accountUserIds.has(String(user._id)) || chatters.some((chatter) =>
...
export async function trackWatchTime(): Promise<WatchTimeResult> {
  ...
  const usersWithTwitch = await usersCol
    .find({
      $or: [
        { twitchLogin: { $exists: true, $ne: null } },
        { twitchUserId: { $exists: true, $ne: null } },
        { _id: { $in: linkedAccounts.rawUserIds } },
      ],
    })
    .toArray();

  const matchedUsers = usersWithTwitch.filter((u) =>
    userMatchesChatter(u, chatters, linkedAccounts.userIds)
  );
  ...
  for (const user of matchedUsers) {
    await progressCol.updateOne(
      { userId: user._id.toString(), challengeId: "watchtime-30" },
      {
        $inc: { progress: INCREMENT_MINUTES },
        $setOnInsert: {
          userId: user._id.toString(),
          challengeId: "watchtime-30",
          createdAt: new Date(),
        },
      },
      { upsert: true }
    ...
    await progressCol.updateOne(
      { userId: user._id.toString(), challengeId: "community-watchtime-1000" },
      {
        $inc: { progress: INCREMENT_MINUTES },
        $setOnInsert: {
          userId: user._id.toString(),
          challengeId: "community-watchtime-1000",
          createdAt: new Date(),
        },
      },
      { upsert: true }
