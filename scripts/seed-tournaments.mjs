...
const tournamentSeeds = [
	{
		...
		description:
			"Ein ARAM-MAYHEM-Turnier ohne LP-Druck und Preise: Es geht um Ruhm, Ehre, dumme Plays, schöne Plays und jede Menge gemeinsames Chaos. Melde dich allein an oder bring dein festes 5er-Team mit.",
...
try {
	...
	for (const seed of tournamentSeeds) {
		...
		console.log(result.upsertedCount ? `Angelegt: ${seed.title}` : `Bereits vorhanden: ${seed.title}`);
	}
} catch (error) {
