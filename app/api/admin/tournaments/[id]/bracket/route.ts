...
import { createId, createRoundRobinMatches, createSingleEliminationMatches } from "@/lib/tournament-engine";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
	...
	if (tournament.bracketType === "single_elimination" && body.stage !== "playoffs") return NextResponse.json({ error: "Dieses Turnier hat keine Gruppenphase. Bitte erstelle den Playoff-Baum." }, { status: 400 });
	...
	if (tournament.maxTeams === 4 && teams.length !== 4) return NextResponse.json({ error: "Fuer dieses Turnier werden genau vier Teams fuer die Halbfinals benoetigt." }, { status: 400 });
	...
	if (body.stage === "playoffs") {
		const matches = createSingleEliminationMatches(id, teams.map((team) => team.id));
	...
	const groups = Array.from({ length: groupCount }, (_, index) => ({ id: createId("group"), tournamentId: id, name: `Gruppe ${String.fromCharCode(65 + index)}`, teamIds: [] as string[], createdAt: new Date() }));
