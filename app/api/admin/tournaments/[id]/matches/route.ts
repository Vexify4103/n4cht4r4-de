...
async function resetLaterPlayoffRounds(db: Db, tournamentId: string, round: number) {
	await db.collection<TournamentMatch>("tournament_matches").updateMany(
		{ tournamentId, stage: "playoff", round: { $gt: round } },
		{
			$set: { teamAId: null, teamBId: null, scoreA: 0, scoreB: 0, winnerTeamId: null, status: "scheduled" },
			$unset: { completedAt: "" },
		},
	);

	const completedMatches = await db.collection<TournamentMatch>("tournament_matches")
		.find({ tournamentId, stage: "playoff", round: { $lte: round }, status: "completed" })
		.sort({ round: 1, position: 1 })
		.toArray();

	for (const completed of completedMatches) {
		if (completed.nextMatchId && completed.nextSlot && completed.winnerTeamId) {
			await db.collection<TournamentMatch>("tournament_matches").updateOne(
				{ id: completed.nextMatchId, tournamentId },
				{ $set: { [completed.nextSlot]: completed.winnerTeamId } },
			);
		}
		const loserTeamId = loserOf(completed);
		if (completed.loserNextMatchId && completed.loserNextSlot && loserTeamId) {
			await db.collection<TournamentMatch>("tournament_matches").updateOne(
				{ id: completed.loserNextMatchId, tournamentId },
				{ $set: { [completed.loserNextSlot]: loserTeamId } },
			);
		}
	}
}
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
	...
	const matches = await db.collection<TournamentMatch>("tournament_matches").find({ tournamentId: id }).project({ _id: 0 }).sort({ stage: 1, groupId: 1, round: 1, position: 1 }).toArray();
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
	...
	if (body?.action === "pairings") {
		...
		if (!tournament) return NextResponse.json({ error: "Turnier nicht gefunden." }, { status: 404 });
		id = String(tournament.id);
		const firstRoundMatches = await db.collection<TournamentMatch>("tournament_matches")
			.find({ tournamentId: id, stage: "playoff", round: 1, placement: { $ne: "third_place" } })
			.sort({ position: 1 })
			.toArray();
		...
		if (
			normalized.some((pairing: { matchId: string; teamAId: string; teamBId: string }) => !matchIds.has(pairing.matchId) || !pairing.teamAId || !pairing.teamBId || pairing.teamAId === pairing.teamBId)
		...
		await db.collection<TournamentMatch>("tournament_matches").bulkWrite(normalized.map((pairing: { matchId: string; teamAId: string; teamBId: string }) => ({
			updateOne: {
		...
		await resetLaterPlayoffRounds(db, id, 1);
	...
	if (body?.action === "schedule") {
		if (typeof body.matchId !== "string" || typeof body.scheduledAt !== "string" || Number.isNaN(Date.parse(body.scheduledAt))) return NextResponse.json({ error: "Bitte wähle einen gültigen Termin." }, { status: 400 });
		await client.connect();
		const db = client.db();
		const tournament = await resolveTournament(db, id);
		if (!tournament) return NextResponse.json({ error: "Turnier nicht gefunden." }, { status: 404 });
		id = String(tournament.id);
		const result = await db.collection<TournamentMatch>("tournament_matches").findOneAndUpdate({ id: body.matchId, tournamentId: id }, { $set: { scheduledAt: new Date(body.scheduledAt) } }, { returnDocument: "after", projection: { _id: 0 } });
	...
	if (typeof body?.matchId !== "string" || !Number.isInteger(body.scoreA) || !Number.isInteger(body.scoreB) || body.scoreA === body.scoreB || body.scoreA < 0 || body.scoreB < 0) return NextResponse.json({ error: "Bitte gib ein gültiges Ergebnis ohne Unentschieden an." }, { status: 400 });
	await client.connect();
	const db = client.db();
	...
	if (Math.max(body.scoreA, body.scoreB) !== winsNeeded || Math.min(body.scoreA, body.scoreB) >= winsNeeded) return NextResponse.json({ error: `Dieses Match ist Best of ${seriesBestOf}. Das Siegerteam braucht genau ${winsNeeded} Maps.` }, { status: 400 });
	...
	if (match.winnerTeamId && match.winnerTeamId !== winnerTeamId) await resetLaterPlayoffRounds(db, id, match.round);
	if (match.nextMatchId && match.nextSlot) await db.collection<TournamentMatch>("tournament_matches").updateOne({ id: match.nextMatchId }, { $set: { [match.nextSlot]: winnerTeamId } });
	const loserTeamId = winnerTeamId === match.teamAId ? match.teamBId : match.teamAId;
	if (match.loserNextMatchId && match.loserNextSlot) await db.collection<TournamentMatch>("tournament_matches").updateOne({ id: match.loserNextMatchId }, { $set: { [match.loserNextSlot]: loserTeamId } });
	if (match.stage === "group" && match.groupId) await rebuildGroupStandings(db, id, match.groupId);
	if (match.stage === "playoff") {
		const openPlayoffMatches = await db.collection<TournamentMatch>("tournament_matches").countDocuments({ tournamentId: id, stage: "playoff", status: { $ne: "completed" } });
