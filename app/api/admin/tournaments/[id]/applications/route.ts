...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
	...
	const [applications, wishGroups] = await Promise.all([
		db.collection("tournament_applications").find({ tournamentId: id }).project({ _id: 0 }).sort({ createdAt: -1 }).toArray(),
		db.collection("tournament_wish_groups").find({ tournamentId: id }).project({ _id: 0 }).sort({ createdAt: 1 }).toArray(),
	]);
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
	...
	if (typeof body?.applicationId !== "string" || !["pending", "accepted", "waitlisted", "rejected"].includes(body.status))
		return NextResponse.json({ error: "Ungültige Bewerbungsänderung." }, { status: 400 });
	await client.connect();
	const db = client.db();
	const tournament = await resolveTournament(db, id);
	if (!tournament) return NextResponse.json({ error: "Turnier nicht gefunden." }, { status: 404 });
	...
	const result = await db
		...
		.findOneAndUpdate(
			{ id: body.applicationId, tournamentId: id },
			{ $set: { status: body.status, reviewedBy: staff.userId, reviewedAt: new Date(), reviewerNote: typeof body.note === "string" ? body.note.slice(0, 1000) : "" } },
